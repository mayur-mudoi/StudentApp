...
export default function Home() {
  ...
  const userAttendanceRecords = useMemo(() => {
    ...
    return attendanceData.filter(
      (rec) =>
        ...
        rec.Student_Id === studentData.$id ||
        (typeof rec.Student_Id === 'object' && rec.Student_Id.$id === studentData.$id)
    );
  ...
  useEffect(() => {
    ...
    console.log('userAttendanceRecords:', userAttendanceRecords);
  }, [attendanceData, studentData, userAttendanceRecords]);
  ...
  // Prepare attendance data for CalendarComponent
  const attendanceForCalendar = userAttendanceRecords.reduce<Record<string, string>>((acc, rec) => {
    const date = (rec.Date || rec.Marked_at)?.slice(0, 10);
    if (date) acc[date] = rec.Status === 'Present' || rec.status === 'present' ? 'Present' : 'Absent';
    return acc;
  }, {});
  console.log('attendanceForCalendar:', attendanceForCalendar);
  ...
  // Fix linter errors: add types for 'day' and 'status' parameters
  const getDateStatus = (day: number): string => {
        ...
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.headerContent}>
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push('/src/Profile/ProfileScreen')}
            >
        ...
        <View style={styles.innerContent}>
          ...
          <View style={styles.calendarContainer}>
            <View style={styles.calendarHeader}>
              ...
              >
                <Calendar color="#ffffff" size={Math.max(20, width * 0.04)} />
              </TouchableOpacity>
